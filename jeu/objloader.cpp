...
int objloader::load(const char* filename)
{
                                                                                                                                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                                                                                                                                        //all of the materials
        {
				...
				char filen2[200] = "D:/Modeles 3D/";

				strcat(&filen2[0],&filen[0]);
                std::ifstream mtlin(filen2);     //open the file
				std::cout << "File : " << filen2 << std::endl;
                ...
                for(int j=0;j<tmp.size();j++) //go through all lines of the mtllib file
                {
					//std::cout << "|" << tmp[j] << std::endl;
                        if(tmp[j][0]=='#')      //we don't care about comments
                                continue;
                        if(tmp[j][0]=='n' && tmp[j][1]=='e' && tmp[j][2]=='w')  //new material
                        {
                                ...
                                sscanf(tmp[j].c_str(),"newmtl %s",name);        //read in the name
                        }else if(tmp[j][0]=='N' && tmp[j][1]=='s')      //the shininess
                        {
                                sscanf(tmp[j].c_str(),"Ns %f",&ns);
                                ismat=true;
                        }else if(tmp[j][0]=='K' && tmp[j][1]=='a')      //the ambient
                        {
                                sscanf(tmp[j].c_str(),"Ka %f %f %f",&amb[0],&amb[1],&amb[2]);
                                ismat=true;
                        }else if(tmp[j][0]=='K' && tmp[j][1]=='d')      //the diffuse
                        {
                                sscanf(tmp[j].c_str(),"Kd %f %f %f",&dif[0],&dif[1],&dif[2]);
                                ismat=true;
                        }else if(tmp[j][0]=='K' && tmp[j][1]=='s')      //the specular
                        {
                                sscanf(tmp[j].c_str(),"Ks %f %f %f",&spec[0],&spec[1],&spec[2]);
                                ismat=true;
                        }else if(tmp[j][0]=='N' && tmp[j][1]=='j')      //the j don't know what is this
                        {
                                sscanf(tmp[j].c_str(),"Ni %f",&ni);
                                ismat=true;
                        }else if(tmp[j][0]=='d' && tmp[j][1]==' ')      //the alpha
                        {
                                sscanf(tmp[j].c_str(),"d %f",&alpha);
                                ismat=true;
                        }else if(tmp[j][0]=='j' && tmp[j][1]=='l')      //the illum (don't ask)
                        {
                                sscanf(tmp[j].c_str(),"illum %d",&illum);
                                ismat=true;
                        }else if(tmp[j][0]=='m' && tmp[j][1]=='a')      //and the texture
                        {
