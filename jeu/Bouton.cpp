...
Bouton::Bouton(std::string texte, sint32 x, sint32 y, sint32 code)
{
	...
	this->conteneurTextures.ajouter("boutons\\bouton_pas_survole.bmp");
	this->conteneurTextures.ajouter("boutons\\bouton_survole.bmp");
	this->conteneurTextures.ajouter("boutons\\bouton_enfonce.bmp");
}


Bouton::~Bouton()
{
	// Libération de la texture du texte du bouton
	glDeleteTextures(1, &this->textureTexte);

	this->conteneurTextures.supprimer("boutons\\bouton_pas_survole.bmp");
	this->conteneurTextures.supprimer("boutons\\bouton_survole.bmp");
	this->conteneurTextures.supprimer("boutons\\bouton_enfonce.bmp");
}

GLuint Bouton::creerTextureTexte(std::string texte)
{
	// Chargement de la police du bouton
	TTF_Font* police = TTF_OpenFont("polices\\Cartonsix NC.ttf", 35);
	if (NULL == police)
	{
...
void Bouton::dessiner(void)
{
	...
	{
		case PAS_SURVOLE:
			this->dessinerFond("boutons\\bouton_pas_survole.bmp");
			break;

		case SURVOLE:
			this->dessinerFond("boutons\\bouton_survole.bmp");
			break;

		case ENFONCE:
			this->dessinerFond("boutons\\bouton_enfonce.bmp");
			break;
	}
...
Bouton::Etat Bouton::lectureEtatBouton(void)
{
	...
	uint32 largeurBouton = this->conteneurTextures.texture("boutons\\bouton_pas_survole.bmp").largeur;
	uint32 hauteurBouton = this->conteneurTextures.texture("boutons\\bouton_pas_survole.bmp").hauteur;
